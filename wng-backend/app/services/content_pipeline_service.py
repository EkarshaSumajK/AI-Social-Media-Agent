    ...
    async def _generate_draft_content(
        ...
        *,
        ...
        guidance = (
            ...
            'synonyms, and reorganize the order of ideas. '
            'CRITICAL HUMANIZATION: The output MUST pass AI-detection tools as human-written. '
            'Use contractions throughout (we\'re, don\'t, can\'t, it\'s, won\'t, they\'re) — at least 3-5 per section. '
            'NEVER use these AI-giveaway phrases: "It is important to", "It is essential to", "It is crucial", '
            '"This can be", "By doing this", "By understanding", "Together, we can", "In today\'s world", '
            '"Let\'s explore", "Remember that", "Take the first step", "We provide a comprehensive approach". '
            'Vary sentence lengths aggressively — mix 5-word fragments with 20-word sentences. '
            'Never start two consecutive sentences the same way. '
            'Include rhetorical questions naturally. Drop occasional filler words humans use (actually, honestly, really, just). '
            'Write like a warm clinician chatting with a parent, not a polished corporate article. '
            'Avoid perfect parallel structure in lists — real writers don\'t repeat the same pattern. '
            'Use first-person voice naturally but not in every sentence. Avoid keyword stuffing, '
